...
export function VehicleGallery({ images, title, locale="pl" }: { images: string[]; title: string; locale?:Locale }) {
  ...
  return <div className="vehicle-gallery">
    <button type="button" className="detail-image vehicle-gallery-main" onClick={()=>setOpen(true)} aria-label={en?'Open fullscreen gallery':'Otwórz galerię pełnoekranową'}>
      <img src={safeImages[active]} alt={`${title} — ${en?'photo':'zdjęcie'} ${active+1}`}/>
      <span className="gallery-count">{active+1} / {safeImages.length}</span>
      <span className="gallery-open-label">{en?'Fullscreen':'Pełny ekran'}</span>
    </button>
    {safeImages.length>1&&<div className="vehicle-thumbs">{safeImages.map((src,index)=><button type="button" className={`vehicle-thumb ${index===active?'active':''}`} key={`${src}-${index}`} onClick={()=>setActive(index)} aria-label={`${en?'Show photo':'Pokaż zdjęcie'} ${index+1}`}><img src={src} alt=""/></button>)}</div>}
