...
export function VehicleCard({ vehicle, locale = "pl" }: { vehicle: Vehicle; locale?: Locale }) {
  ...
  return <article className="vehicle-card">
    ...
    <div className="card-body">
      ...
      <div className="price-row"><div className="price"><strong>{formatPln(vehicle.priceNet)}</strong><small>{en ? "Net" : "Netto"} · {formatPln(grossFromNet(vehicle.priceNet, vehicle.vatRate))} {en ? "gross" : "brutto"}</small></div><Link className="arrow-link" href={`/samochody/${vehicle.slug}`} aria-label={`${en ? "View" : "Zobacz"} ${vehicle.title}`}><Icon name="arrow"/></Link></div>
    </div>
  </article>
}
