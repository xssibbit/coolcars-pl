...
const schema=z.object({
  vehicleId:z.string().min(1),name:z.string().min(2),email:z.string().email().nullable().optional(),phone:z.string().optional(),message:z.string().min(5),
...
export async function POST(request:Request){
  ...
  const inquiry=await db.$transaction(async tx=>{
    const created=await tx.inquiry.create({data:{vehicleId,name,email:email??null,phone,message,userId:user?.id,visitorId,source,medium,campaign,firstSource,firstMedium,firstCampaign,landingPath}});
