...
export async function POST(_:Request,{params}:{params:Promise<{vehicleId:string}>}){
...
export async function DELETE(_:Request,{params}:{params:Promise<{vehicleId:string}>}){
  const user=await getCurrentUser();
  if(!user)return NextResponse.json({error:'Unauthorized'},{status:401});
  const {vehicleId}=await params;
  await db.favorite.deleteMany({where:{userId:user.id,vehicleId}});
  return NextResponse.json({ok:true});
}
